...
char *sftppath = "/tmp/TEST";
char fileName[128] = {'D','o','w','n','l','o','a','d','/'};
char *ipStr;
...
void getName(char* name)
{
    ...
    for(i=strlen(name);i && name[i]!='/' ;i--)
    for(j=i;j<strlen(name);j++)
    {
        tmpFileName[j-i]=name[j];
    }
    ...
    {
        fileName[st]=tmpFileName[i];
        st++;
    }

    printf("Download file to [%s]\n",fileName);
} 
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        if(rc > 0) {
            // int fileTmp=open("notexits.txt",O_RDWR|O_CREAT|S_IRWXU);
            // write(fileTmp, mem, rc);
            // close(fileTmp);
            // write(1, mem, rc);
            getName(sftppath);
